...
const INITIAL_MKS = [
  {
    ...
    dosen_name: 'Dra. Sri Wahyuni, M.Ak.',
    ...
    scoringData: {
      's1': {
        ...
        'k3': { scores: { K: 3, A: 4, W: 3 }, feedbacks: { K: 'Jawaban benar.', A: 'Analisis tajam.', W: 'Cepat.' }, rawScore: 85, status: 'PUBLISHED' },
        'k4': { scores: { E: 4, P: 4, I: 3, C: 4 }, feedbacks: { E: 'Evaluasi mendalam.', P: 'Prediksi logis.', I: 'Penerapan terstruktur.', C: 'Audit mandiri baik.' }, rawScore: 93, status: 'PUBLISHED' },
      ...
      's2': {
        'k1': { scores: { E: 4, P: 4, I: 4, C: 3 }, feedbacks: { E: 'Evaluasi tajam.', P: 'Prediksi akurat.', I: 'Penerapan rapi.', C: 'Refleksi baik.' }, rawScore: 93, status: 'PUBLISHED' },
        'k2': { scores: { K: 4, A: 3, W: 4 }, feedbacks: { K: 'Sangat baik.', A: 'Analisis cukup.', W: 'Tepat waktu.' }, rawScore: 90, status: 'PUBLISHED' },
        'k3': { scores: { K: 4, A: 4, W: 3 }, feedbacks: { K: 'Presisi tinggi.', A: 'Analisis bagus.', W: 'Cepat.' }, rawScore: 93, status: 'PUBLISHED' },
        ...
        'k5': { scores: { T: 3, P: 4, A: 4 }, feedbacks: { T: 'Teori baik.', P: 'Standar presisi.', A: 'Analisis kasus mendalam.' }, rawScore: 90, status: 'PUBLISHED' },
      ...
      's3': {
        ...
        'k3': { scores: { K: 3, A: 2, W: 3 }, feedbacks: { K: 'Cukup baik.', A: 'Perlu latihan analisis.', W: 'Cepat.' }, rawScore: 70, status: 'PUBLISHED' },
      ...
      's4': {
        'k1': { scores: { E: 4, P: 3, I: 3, C: 4 }, feedbacks: { E: 'Mendalam.', P: 'Prediksi logis.', I: 'Penerapan baik.', C: 'Kritik tajam.' }, rawScore: 85, status: 'PUBLISHED' },
        'k2': { scores: { K: 4, A: 4, W: 3 }, feedbacks: { K: 'Tepat.', A: 'Analisis mendalam.', W: 'Cepat.' }, rawScore: 90, status: 'PUBLISHED' },
        'k3': { scores: { K: 3, A: 3, W: 4 }, feedbacks: { K: 'Benar.', A: 'Analisis baik.', W: 'Sangat cepat.' }, rawScore: 80, status: 'PUBLISHED' },
        'k4': { scores: { E: 4, P: 3, I: 4, C: 3 }, feedbacks: { E: 'Evaluasi kuat.', P: 'Prediksi baik.', I: 'Efisien.', C: 'Refleksi ada.' }, rawScore: 85, status: 'PUBLISHED' },
        'k5': { scores: { T: 4, P: 3, A: 3 }, feedbacks: { T: 'Menguasai teori.', P: 'Sesuai standar.', A: 'Analisis cukup.' }, rawScore: 83, status: 'PUBLISHED' },
        'k6': { scores: { T: 4, P: 4, A: 3 }, feedbacks: { T: 'Teori kuat.', P: 'Penerapan baik.', A: 'Analisis tajam.' }, rawScore: 90, status: 'PUBLISHED' },
      },
      's5': {
        'k1': { scores: { E: 3, P: 2, I: 3, C: 2 }, feedbacks: { E: 'Cukup.', P: 'Perlu latihan prediksi.', I: 'Penerapan dasar.', C: 'Refleksi kurang.' }, rawScore: 65, status: 'PUBLISHED' },
        'k2': { scores: { K: 3, A: 2, W: 3 }, feedbacks: { K: 'Cukup.', A: 'Perlu bimbingan.', W: 'Tepat waktu.' }, rawScore: 70, status: 'PUBLISHED' },
        'k3': { scores: { K: 2, A: 3, W: 2 }, feedbacks: { K: 'Perlu teliti.', A: 'Analisis ada.', W: 'Melewati sedikit waktu.' }, rawScore: 60, status: 'PUBLISHED' },
        'k4': { scores: { E: 3, P: 2, I: 3, C: 2 }, feedbacks: { E: 'Cukup.', P: 'Perlu bantuan.', I: 'Dasar.', C: 'Kurang.' }, rawScore: 65, status: 'PUBLISHED' },
      ...
      's6': {
        ...
        'k3': { scores: { K: 4, A: 3, W: 4 }, feedbacks: { K: 'Presisi.', A: 'Analisis baik.', W: 'Cepat.' }, rawScore: 95, status: 'PUBLISHED' },
      ...
      's7': {
        ...
        'k2': { scores: { K: 3, A: 4, W: 3 }, feedbacks: { K: 'Baik.', A: 'Analisis tajam.', W: 'Tepat waktu.' }, rawScore: 85, status: 'PUBLISHED' },
        'k3': { scores: { K: 3, A: 3, W: 3 }, feedbacks: { K: 'Cukup.', A: 'Baik.', W: 'Tepat.' }, rawScore: 75, status: 'PUBLISHED' },
        'k4': { scores: { E: 4, P: 3, I: 3, C: 3 }, feedbacks: { E: 'Mendalam.', P: 'Logis.', I: 'Baik.', C: 'Terstruktur.' }, rawScore: 80, status: 'PUBLISHED' },
        'k5': { scores: { T: 3, P: 3, A: 4 }, feedbacks: { T: 'Baik.', P: 'Sesuai.', A: 'Analisis tajam.' }, rawScore: 80, status: 'PUBLISHED' },
...
const loadSavedMKs = () => {
  try {
    const saved = localStorage.getItem('epic_mks_v3');
    if (!saved) return INITIAL_MKS;
    const parsed = JSON.parse(saved);
    const mk1 = parsed.find(mk => mk.id === 'mk-1');
    // If scoringData is missing for s2, force re-initialize with INITIAL_MKS
    if (!mk1?.scoringData?.['s2']) {
      localStorage.setItem('epic_mks_v3', JSON.stringify(INITIAL_MKS));
      return INITIAL_MKS;
    }
    return parsed;
  } catch (e) {
...
const saveMKsToStorage = (mks) => {
  try {
    localStorage.setItem('epic_mks_v3', JSON.stringify(mks));
  } catch (e) {}
};
...
export const useMKStore = create((set, get) => ({
  ...
  createMK: (mkData, customKomponen = null) => {
    ...
    const sumBobot = komponenToUse.reduce((acc, k) => acc + (k.bobot || 0), 0);
    const allRubricAssigned = komponenToUse.length > 0 && komponenToUse.every(k => k.rubricId);
    const autoStatus = (Math.abs(sumBobot - 1.0) <= 0.001 && allRubricAssigned) ? 'ACTIVE' : 'DRAFT';
    ...
    const newMK = {
      ...
      dosen_name: mkData.dosen_name || 'Dra. Sri Wahyuni, M.Ak.',
  ...
  updateKomponenList: (mkId, newKomponenList) => {
    ...
    // Check if total weight == 1.0 and all have rubrics for auto-activation
    const totalBobot = newKomponenList.reduce((sum, k) => sum + Number(k.bobot || 0), 0);
    const allHaveRubric = newKomponenList.every(k => k.rubricId);
    const is100Percent = Math.abs(totalBobot - 1.0) <= 0.001;

    const updated = mkList.map(mk => {
      if (mk.id === mkId) {
        let newStatus = mk.status;
        if (mk.status === 'DRAFT' && is100Percent && allHaveRubric && newKomponenList.length > 0) {
          newStatus = 'ACTIVE';
        }
        return {
          ...mk,
